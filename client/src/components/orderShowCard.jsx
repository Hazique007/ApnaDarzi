...
const OrderShowCard = ({ order }) => {
    ...
    const fetchOrderData = async () => {
      try {
        const response = await axios.get(
          "https://apnadarzi-311.onrender.com/orders/getorder"
        );
  ...
  if (loading) {
    return (
      <div className="w-full flex justify-center h-full text-[12px]  items-center">
        <BeatLoader size={20} color="#ff58e6" />
      </div>
    );
  }
  ...
  const productImage = productData?.images?.[0]
    ? `https://apnadarzi-311.onrender.com/uploads/${productData.images[0]}`
